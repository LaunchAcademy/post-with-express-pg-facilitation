...
const pool = new pg.Pool({
  connectionString:
    "postgres://postgres:password@localhost:5432/hobbies_development"
})
...
class Hobby {
  ...
  static async findById(id) {
    try {
      const result = await pool.query("SELECT * FROM hobbies WHERE id = $1;", [
        id
      ])
  ...
  async save() {
    try {
      const query =
        "INSERT INTO hobbies (name, description) VALUES ($1, $2) RETURNING id;"
      const result = await pool.query(query, [this.name, this.description])
      // "INSERT INTO hobbies (name, description) VALUES ('', '') RETURNING id;"
